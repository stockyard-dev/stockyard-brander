...
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.type||'').toLowerCase().includes(q)||(i.format||'').toLowerCase().includes(q)||(i.url||'').toLowerCase().includes(q)});
...
if(i.format)h+='<span>'+esc(i.format)+'</span>';
...
function formHTML(item){var i=item||{name:"",type:"",format:"",url:"",version:"",status:"",notes:""};var isEdit=!!item;
...
h+='<div class="fr"><label>Format</label><input id="f-format" value="'+esc(i.format)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),type:document.getElementById('f-type').value.trim(),format:document.getElementById('f-format').value.trim(),url:document.getElementById('f-url').value.trim(),version:document.getElementById('f-version').value.trim(),status:document.getElementById('f-status').value.trim(),notes:document.getElementById('f-notes').value.trim()};
